...
#pragma omp simd
    for (int i = 0; i < E; i++) {
        blk_dist[edge[i].src * VP + edge[i].dst] = edge[i].w;
    }
    ...
    for (int k = 0, nk = nblocks - 1; k < nblocks; k++, nk--) {
        /* Phase 1 */
        proc_1_glob<<<1, blk>>>(blk_dist_dev, k, int_pitch);
        /* Phase 2 */
        proc_2_glob<<<dim3(nblocks - 1, 2), blk>>>(blk_dist_dev, 0, k, int_pitch);
        /* Phase 3 */
        proc_3_glob<<<dim3(nblocks - 1, nblocks - 1), blk>>>(blk_dist_dev, 0, 0, k, int_pitch);
    }
    ...
    /* Copy result to dist */
    TIMING_START(copy_result);
    for (int i = 0; i < V; i++) {
#pragma omp simd
        for (int j = 0; j < V; j++) {
            dist[i * V + j] = min(blk_dist[i * VP + j], INF);
        }
    }
    TIMING_END(copy_result);
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_k_sm[r + rr * half_bs][c + cc * half_bs] = _ref(k, k, r + rr * half_bs, c + cc * half_bs);
        }
    }
    __syncthreads();
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                k_k_sm[r + rr * half_bs][c + cc * half_bs] = min(k_k_sm[r + rr * half_bs][c + cc * half_bs], k_k_sm[r + rr * half_bs][b] + k_k_sm[b][c + cc * half_bs]);
            }
        }
        __syncthreads();
    }
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(k, k, r + rr * half_bs, c + cc * half_bs) = k_k_sm[r + rr * half_bs][c + cc * half_bs];
        }
    }
}
__global__ void proc_2_glob(int *blk_dist, int s, int k, int pitch) {
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                sm[r + rr * half_bs][c + cc * half_bs] = _ref(i, k, r + rr * half_bs, c + cc * half_bs);
            }
        }
        __syncthreads();
...
#pragma unroll
                for (int cc = 0; cc < div_block; cc++) {
                    sm[r + rr * half_bs][c + cc * half_bs] = min(sm[r + rr * half_bs][c + cc * half_bs], sm[r + rr * half_bs][b] + k_k_sm[b][c + cc * half_bs]);
                }
            }
            __syncthreads();
        }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                _ref(i, k, r + rr * half_bs, c + cc * half_bs) = sm[r + rr * half_bs][c + cc * half_bs];
            }
        }
    } else {
        /* cols */
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                sm[r + rr * half_bs][c + cc * half_bs] = _ref(k, i, r + rr * half_bs, c + cc * half_bs);
            }
        }
        __syncthreads();
...
#pragma unroll
                for (int cc = 0; cc < div_block; cc++) {
                    sm[r + rr * half_bs][c + cc * half_bs] = min(sm[r + rr * half_bs][c + cc * half_bs], k_k_sm[r + rr * half_bs][b] + sm[b][c + cc * half_bs]);
                }
            }
            __syncthreads();
        }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                _ref(k, i, r + rr * half_bs, c + cc * half_bs) = sm[r + rr * half_bs][c + cc * half_bs];
            }
        }
    }
}
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_k_sm[r + rr][c + cc] = _ref(i, k, r + rr, c + cc);
        }
    }
#pragma unroll
    for (int rr = 0; rr < div_block; rr++) {
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_j_sm[r + rr][c + cc] = _ref(k, j, r + rr, c + cc);
        }
    }
    __syncthreads();
#pragma unroll
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                loc[rr][cc] = min(loc[rr][cc], i_k_sm[r + rr][b] + k_j_sm[b][c + cc]);
            }
        }
    }
#pragma unroll
