...
int main(int argc, char **argv) {
    ...
    for (int k = 0, nk = nblocks - 1; k < nblocks; k++, nk--) {
        /* Phase 1 */
        proc_1_glob<<<1, blk>>>(blk_dist_dev, k, nblocks, pitch);
        /* Phase 2 */
        if (k)
            proc_2_glob<<<k, blk>>>(blk_dist_dev, 0, k, nblocks, pitch);
        if (nk)
            proc_2_glob<<<nk, blk>>>(blk_dist_dev, k + 1, k, nblocks, pitch);
        /* Phase 3 */
        if (k)
            proc_3_glob<<<dim3(k, k), blk>>>(blk_dist_dev, 0, 0, k, nblocks, pitch);
        if (k && nk)
            proc_3_glob<<<dim3(nk, k), blk>>>(blk_dist_dev, 0, k + 1, k, nblocks, pitch);
        if (k && nk)
            proc_3_glob<<<dim3(k, nk), blk>>>(blk_dist_dev, k + 1, 0, k, nblocks, pitch);
        if (nk)
            proc_3_glob<<<dim3(nk, nk), blk>>>(blk_dist_dev, k + 1, k + 1, k, nblocks, pitch);
    }
...
#define _ref(i, j, r, c) blk_dist[(i * block_size + r) * (pitch >> 2) + j * block_size + c]
__global__ void proc_1_glob(int *blk_dist, int k, int nblocks, int pitch) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_k_sm[r + rr][c + cc] = _ref(k, k, r + rr, c + cc);
        }
    }
    __syncthreads();
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(k, k, r + rr, c + cc) = k_k_sm[r + rr][c + cc];
        }
    }
}
__global__ void proc_2_glob(int *blk_dist, int s, int k, int nblocks, int pitch) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(i, k, r + rr, c + cc) = i_k_sm[r + rr][c + cc];
            _ref(k, j, r + rr, c + cc) = k_j_sm[r + rr][c + cc];
        }
    }
}
__global__ void proc_3_glob(int *blk_dist, int s_i, int s_j, int k, int nblocks, int pitch) {
