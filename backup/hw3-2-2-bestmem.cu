...
__global__ void proc_2_glob(int *blk_dist, int k, int pitch);
__global__ void proc_3_glob(int *blk_dist, int k, int pitch);
...
int main(int argc, char **argv) {
    ...
    for (int k = 0, nk = nblocks - 1; k < nblocks; k++, nk--) {
        ...
        /* Phase 2 */
        proc_2_glob<<<nblocks, blk>>>(blk_dist_dev, k, int_pitch);
        /* Phase 3 */
        proc_3_glob<<<dim3(nblocks, nblocks), blk>>>(blk_dist_dev, k, int_pitch);
    }
