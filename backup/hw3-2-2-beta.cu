...
int main(int argc, char **argv) {
    ...
    for (int k = 0, nk = nblocks - 1; k < nblocks; k++, nk--) {
        ...
        /* Phase 2 */
        proc_2_glob<<<nblocks, blk>>>(blk_dist_dev, 0, k, int_pitch);
        /* Phase 3 */
        proc_3_glob<<<dim3(nblocks, nblocks), blk>>>(blk_dist_dev, 0, 0, k, int_pitch);
    }
...
__global__ void proc_2_glob(int *blk_dist, int s, int k, int pitch) {
    ...
    const int i = s + blockIdx.x;
    const int j = s + blockIdx.x;
    const int r = threadIdx.y;
    const int c = threadIdx.x;
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_k_sm[r + rr * half_bs][c + cc * half_bs] = _ref(i, k, r + rr * half_bs, c + cc * half_bs);
        }
    }
#pragma unroll
    for (int rr = 0; rr < div_block; rr++) {
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_j_sm[r + rr * half_bs][c + cc * half_bs] = _ref(k, j, r + rr * half_bs, c + cc * half_bs);
        }
    }
#pragma unroll
    for (int rr = 0; rr < div_block; rr++) {
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                i_k_sm[r + rr * half_bs][c + cc * half_bs] = min(i_k_sm[r + rr * half_bs][c + cc * half_bs], i_k_sm[r + rr * half_bs][b] + k_k_sm[b][c + cc * half_bs]);
            }
        }
        for (int rr = 0; rr < div_block; rr++) {
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                k_j_sm[r + rr * half_bs][c + cc * half_bs] = min(k_j_sm[r + rr * half_bs][c + cc * half_bs], k_k_sm[r + rr * half_bs][b] + k_j_sm[b][c + cc * half_bs]);
            }
        }
        __syncthreads();
    }
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(i, k, r + rr * half_bs, c + cc * half_bs) = i_k_sm[r + rr * half_bs][c + cc * half_bs];
        }
    }
#pragma unroll
    for (int rr = 0; rr < div_block; rr++) {
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(k, j, r + rr * half_bs, c + cc * half_bs) = k_j_sm[r + rr * half_bs][c + cc * half_bs];
        }
    }
}
__global__ void proc_3_glob(int *blk_dist, int s_i, int s_j, int k, int pitch) {
    ...
    const int i = s_i + blockIdx.y;
    const int j = s_j + blockIdx.x;
    const int r = threadIdx.y;
    const int c = threadIdx.x;
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            loc[rr][cc] = _ref(i, j, r + rr * half_bs, c + cc * half_bs);
        }
    }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                loc[rr][cc] = min(loc[rr][cc], i_k_sm[r + rr * half_bs][b] + k_j_sm[b][c + cc * half_bs]);
            }
        }
    }
#pragma unroll
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            _ref(i, j, r + rr * half_bs, c + cc * half_bs) = loc[rr][cc];
        }
    }
}