...
#define _idx(i, j, r, c) ((((i)*block_size + (r)) * nblocks) * block_size + j * block_size + (c))
__global__ void proc_1_glob(int *blk_dist, int k, int nblocks) {
    __shared__ int k_k_sm[block_size][block_size];

    const int r = threadIdx.y * div_block;
    const int c = threadIdx.x * div_block;
    int tmp;
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_k_sm[r + rr][c + cc] = blk_dist[_idx(k, k, r + rr, c + cc)];
        }
    }
    __syncthreads();
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            blk_dist[_idx(k, k, r + rr, c + cc)] = k_k_sm[r + rr][c + cc];
        }
    }
}
__global__ void proc_2_glob(int *blk_dist, int s, int k, int nblocks) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_k_sm[r + rr][c + cc] = blk_dist[_idx(i, k, r + rr, c + cc)];
            k_j_sm[r + rr][c + cc] = blk_dist[_idx(k, j, r + rr, c + cc)];
            k_k_sm[r + rr][c + cc] = blk_dist[_idx(k, k, r + rr, c + cc)];
        }
    }
    __syncthreads();
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            blk_dist[_idx(i, k, r + rr, c + cc)] = i_k_sm[r + rr][c + cc];
            blk_dist[_idx(k, j, r + rr, c + cc)] = k_j_sm[r + rr][c + cc];
        }
    }
}
__global__ void proc_3_glob(int *blk_dist, int s_i, int s_j, int k, int nblocks) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            loc[rr][cc] = blk_dist[_idx(i, j, r + rr, c + cc)];
        }
    }
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            blk_dist[_idx(i, j, r + rr, c + cc)] = loc[rr][cc];
        }
    }
}