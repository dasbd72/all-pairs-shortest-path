...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_k_sm[r + rr * half_block_size][c + cc * half_block_size] = k_k_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size];
        }
    }
    __syncthreads();
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            k_k_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size] = k_k_sm[r + rr * half_block_size][c + cc * half_block_size];
        }
    }
}
__global__ void proc_2_glob(int *blk_dist, int s, int k, int nblocks) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_k_sm[r + rr * half_block_size][c + cc * half_block_size] = i_k_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size];
            k_j_sm[r + rr * half_block_size][c + cc * half_block_size] = k_j_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size];
            k_k_sm[r + rr * half_block_size][c + cc * half_block_size] = k_k_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size];
        }
    }
    __syncthreads();
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_k_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size] = i_k_sm[r + rr * half_block_size][c + cc * half_block_size];
            k_j_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size] = k_j_sm[r + rr * half_block_size][c + cc * half_block_size];
        }
    }
}
__global__ void proc_3_glob(int *blk_dist, int s_i, int s_j, int k, int nblocks) {
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            loc[rr][cc] = i_j_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size];
        }
    }
...
#pragma unroll
        for (int cc = 0; cc < div_block; cc++) {
            i_j_ptr[(r + rr * half_block_size) * block_size + c + cc * half_block_size] = loc[rr][cc];
        }
    }
}