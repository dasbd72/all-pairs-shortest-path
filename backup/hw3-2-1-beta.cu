...
int main(int argc, char **argv) {
    ...
    for (int k = 0, nk = nblocks - 1; k < nblocks; k++, nk--) {
        ...
        /* Phase 2 */
        proc_2_glob<<<dim3(nblocks, 2), blk>>>(blk_dist_dev, 0, k, int_pitch);
        /* Phase 3 */
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                k_k_sm[r + rr][c + cc] = min(k_k_sm[r + rr][c + cc], k_k_sm[r + rr][b] + k_k_sm[b][c + cc]);
            }
        }
        __syncthreads();
    }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                sm[r + rr][c + cc] = _ref(i, k, r + rr, c + cc);
            }
        }
        __syncthreads();
...
#pragma unroll
                for (int cc = 0; cc < div_block; cc++) {
                    sm[r + rr][c + cc] = min(sm[r + rr][c + cc], sm[r + rr][b] + k_k_sm[b][c + cc]);
                }
            }
            __syncthreads();
        }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                _ref(i, k, r + rr, c + cc) = sm[r + rr][c + cc];
            }
        }
    } else {
        /* cols */
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                sm[r + rr][c + cc] = _ref(k, i, r + rr, c + cc);
            }
        }
        __syncthreads();
...
#pragma unroll
                for (int cc = 0; cc < div_block; cc++) {
                    sm[r + rr][c + cc] = min(sm[r + rr][c + cc], k_k_sm[r + rr][b] + sm[b][c + cc]);
                }
            }
            __syncthreads();
        }
...
#pragma unroll
            for (int cc = 0; cc < div_block; cc++) {
                _ref(k, i, r + rr, c + cc) = sm[r + rr][c + cc];
            }
        }
    }
}
